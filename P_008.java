package eulerProject;

public class P_008 {
    // four adjacent digits in the 1000-digit number that have the greatest product are 9 × 9 × 8 × 9 = 5832
    //Find the thirteen(13) adjacent digits in the 1000-digit number that have the greatest product
    //        2.3514624E10
    //   ans:     23514624000
    public static void main(String[] args) {
        String data = "";
        Float result, max = 0f;


        String[] input = data.split("");

        for (int i = 0; i < input.length - 12; i++) {
            result = Float.parseFloat(input[i]) * Float.parseFloat(input[i + 1]) * Float.parseFloat(input[i + 2]) * Float.parseFloat(input[i + 3])
                    * Float.parseFloat(input[i + 4]) * Float.parseFloat(input[i + 5]) * Float.parseFloat(input[i + 6]) * Float.parseFloat(input[i + 7])
                    * Float.parseFloat(input[i + 8]) * Float.parseFloat(input[i + 9]) * Float.parseFloat(input[i + 10]) * Float.parseFloat(input[i + 11])
                    * Float.parseFloat(input[i + 12]);
            if (result > max) {
                max = result;
                System.out.println("13:   " + input[i] + input[i + 1] + input[i + 2] + input[i + 3] + input[i + 4] + input[i + 5] + input[i + 6] + input[i + 7]
                        + input[i + 8] + input[i + 9] + input[i + 10] + input[i + 11] + input[i + 12]);
                System.out.println(result + "\n");
            }
        }
        System.out.println("Max: " + max);
    }
}
